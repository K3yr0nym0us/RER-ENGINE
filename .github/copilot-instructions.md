...
- **GPU del motor: solo Vulkan** (wgpu, `engine_shared::gpu`). No OpenGL, no fallback a GL, no mezclar backends. DX12 solo futuro en Windows.
- Viewport del juego: **ventana overlay** winit alineada al editor (no embebida en Chromium, no WebGL del front para el mundo).
...
Politica grafica centralizada en `engine_shared/src/gpu.rs` y `engine_shared/src/overlay.rs` (ventana + bounds). No reintroducir `Backends::GL`, `with_embed_parent_window` ni ramas "compatibles con OpenGL".
