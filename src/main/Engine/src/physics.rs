// ---------------------------------------------------------------------------
// Physics — integración con Rapier3D
//
// Mantiene un PhysicsWorld con rigid bodies y colliders.
// Cada frame se llama `step()` y las posiciones se escriben de vuelta
// en los componentes Transform del ECS.
// ---------------------------------------------------------------------------
...
impl PhysicsWorld {
    ...
    /// Añade un rigid body dinámico (cae por gravedad) con colisionador esférico.
    ...
    /// Añade un plano estático (suelo) en y = 0.
    pub fn add_static_ground(&mut self) -> ColliderHandle {
        let collider = ColliderBuilder::halfspace(UnitVector::new_normalize(vector![0.0, 1.0, 0.0]))
            .build();
        self.colliders.insert(collider)
    }

    /// Añade una caja dinámica.
    ...
    pub fn step(&mut self, dt: f32, ecs: &mut World) {
        self.integration_params.dt = dt.clamp(0.0001, 0.05); // máx 50ms por frame
