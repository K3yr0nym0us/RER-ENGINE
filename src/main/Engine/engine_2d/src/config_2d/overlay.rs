...
pub(super) fn create_quad_xy(device: &wgpu::Device, cx: f32, cy: f32, w: f32, h: f32, label: &str) -> Mesh {
    let hw = w / 2.0;
    let hh = h / 2.0;
    let vertices = vec![
...
pub(super) fn build_tool_overlay(device: &wgpu::Device, pts: &[[f32; 2]], cursor: Option<[f32; 2]>) -> gizmo::GizmoBuffer {
    const ARM:         f32 = 0.15;
    const Z:           f32 = 0.1;
