...
impl State {
    ...
    pub(crate) fn create_box_entity_at(
        ...
        self.uv_rects
            .push(self.atlas.pack(&self.queue, &color, 1, 1));
        ...
        self.world
            .insert(entity, MeshComponent { mesh_idx, tex_idx });
        self.world.insert(
            entity,
            Transform {
...
pub(super) fn create_mesh_from_4_points(
    pts: &[[f32; 2]; 4],
    device: &wgpu::Device,
) -> (Mesh, [f32; 3], [f32; 3]) {
    ...
    let scale = [bw, bh, 1.0];
    (
        upload(device, &vertices, &indices, "collider-quad"),
        position,
        scale,
    )
}
