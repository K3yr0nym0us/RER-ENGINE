...
/**
 * Comandos `cmd` permitidos por motor (stdin JSON).
 * Fuente de verdad en Rust: `engine_ipc_common` (común) + `engine_*d/src/engine_command.rs` (solo 2D/3D).
 * Al añadir un comando nuevo, actualizar solo el catálogo del motor correspondiente.
 */

/** Comandos compartidos por ambos binarios (recursos, undo, player UI, escena base, …). */
const ENGINE_COMMANDS_SHARED = [
  'ping',
  'shutdown',
  'set_clear_color',
  'resize',
...
/** Solo motor 2D (`engine_2d/src/ipc.rs`). */
const ENGINE_COMMANDS_2D_ONLY = [
  'load_scenario',
  'set_scenario_scale',
  'load_character',
  'set_character_scale',
...
/** Solo motor 3D (`engine_3d/src/ipc.rs`). */
const ENGINE_COMMANDS_3D_ONLY = [
  'spawn_cached_model',
