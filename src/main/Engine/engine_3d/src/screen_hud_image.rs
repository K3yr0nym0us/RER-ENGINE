//! # Imágenes HUD en pantalla (exclusivo)
//!
//! Este módulo existe **únicamente** para dibujar PNG/UI en coordenadas de pantalla del
//! viewport del motor 3D (espacio NDC con `view_proj` identidad).
//!
//! **No usar** para:
//! - texturas de modelos, suelo, sprites del mundo o `TextureArray` de escena;
//! - gizmos, rejilla, colisiones ni materiales PBR;
//! - empaquetar assets de entidades o el atlas principal del juego.
//!
//! API pública pensada para reutilizar: empaquetar PNG → `ScreenHudPackedImage` →
//! `build_screen_hud_instance` + pass `screen_hud_pipeline`.
...
impl ScreenHudAtlas {
    ...
    fn pack_raw(&mut self, queue: &wgpu::Queue, rgba: &[u8], w: u32, h: u32) -> [f32; 4] {
        ...
        if self.cursor_y + h > self.height {
            log::error!(
                "[screen_hud_image] atlas lleno — no se puede empacar {w}×{h} (solo HUD pantalla)"
            );
...
/// Instancia para `screen_hud_pipeline`. `tex_layer_pad` = UV rect; **no** es índice de capa.
