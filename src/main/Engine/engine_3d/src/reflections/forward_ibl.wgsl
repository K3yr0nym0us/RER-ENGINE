...
/// Devuelve `vec4(rgb, flag)` con `.w = 1.0` si hay override de cubemap activo.
fn forward_sample_metallic_env(
    ...
    if surface_metallic <= 0.5 || probe_layer < 0 {
    ...
    let rgb = textureSampleLevel(t_probe, s_probe, sample_dir, probe_layer, lod).rgb;
